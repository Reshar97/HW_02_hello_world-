...
static char my_str[MAX_STRING_LEN];

static unsigned int idx = 0;

static char ch_val = ' ';

static void update_str_buf(void)
{
...
static int param_ch_val_set(const char *val, const struct kernel_param *kp)
{
    unsigned long temp;
    char *endptr;
    temp = simple_strtoul(val, &endptr, 10);
    if (*endptr == '\0' && temp >= 32 && temp <= 126) { 
        ch_val = (char)temp;
        update_str_buf(); 
    }
    return 0;
...
MODULE_PARM_DESC(my_str, "Read-only parameter showing constructed string");
